...
int DeleteHash(CountryPosition head)
{
	while (head->next != NULL) {
		DeleteAllCities(head->cityHead);
		head = head->next;
	}
	DeleteAllCities(head->cityHead);
}

int DeleteAllCities(CityPosition current)
{
	if (current == NULL) {
		free(current);
		return 0;
	}
	DeleteAllCities(current->left);
	DeleteAllCities(current->right);
}